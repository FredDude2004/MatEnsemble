            ...
            while True:
                ...
                if len(self.pending_tasks) > 0:
                    ...
                    if self.gpus_per_task > 0:
                        ...
                        while (
                            self.free_cores >= self.tasks_per_job * self.cores_per_task
                            and self.free_gpus
                            >= self.tasks_per_job * self.gpus_per_task
                        ...
                        while (
                            ...
                            if dynopro:
                                if self.nnodes is None or self.gpus_per_node is None:
                                    ...
                                    flxt.hetero_job_submit(
                                        executor,
                                        nnodes=self.nnodes,
                                        gpus_per_node=self.gpus_per_node,
                                        command=self.gen_task_cmd,
                                        task=cur_task,
                                        task_args=cur_task_args,
                                        task_directory=cur_task_dir,
                                    )
                            else:
                                flxt.job_submit(
                                    executor,
                ...
                for fut in done:
                    ...
                    try:
                        self.logger.info(
                            f"Task {fut.task_} result: {fut.result(timeout=buffer_time)}"
                        )
                        if fut.result(timeout=buffer_time) != 0:
                            self.logger.debug(
                                f"Task {fut.task_} exited with ERROR CODE {fut.result()}"
                            )
                            self.failed_tasks.append(fut.task_)
                    except Exception as e:
