...
class SuperFluxManager():
    ...
    def progress_monitor(self, completed_tasks, running_tasks, pending_tasks):
        ...
        self.logger.info(
                f"===     {str(completed_tasks).rjust(width, '_')}  |    {str(running_tasks).rjust(width, '_')}  |    {str(pending_tasks).rjust(width, '_')} ==="
        )
            ...
            while True:
                ...
                if len(self.pending_tasks)>0:
                    ...
                    if self.gpus_per_task>0:
                        ...
                        while (
                            self.free_cores >= self.tasks_per_job[0] * self.cores_per_task 
                            and self.free_gpus >= self.tasks_per_job[0] * self.gpus_per_task 
               ...
               # time.sleep(120)
                ...
                for fut in done:
                    ...
                    width = 10
                    self.logger.info(f"=== COMPLETED JOBS === RUNNING JOBS === PENDING JOBS ===")
                    self.logger.info(f"===     {str(len(self.completed_tasks)).rjust(width, '_')}  |    {str(len(self.running_tasks)).rjust(width, '_')}  |    {str(len(self.pending_tasks)).rjust(width, '_')} ===")
